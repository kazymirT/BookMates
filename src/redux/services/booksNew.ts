  ...
  endpoints: (builder) => ({
    getBooks: builder.query<BooksResponse, BooksArgsNew>({
      query: ({
        ...
        url: `book?lang=${lang}&page=${page}&limit=${limit}`,
      }),
    }),
...
// url: `book?lang=${lang}&years=${years.join(',')}&languages=${languages.join(',')}&minPrice=${minPrice}&maxPrice=${maxPrice}&sortPrice=${sortPrice}&new=${isNew}&alphabetical=${alphabetical}&limit=${limit}&page=${page}`,
